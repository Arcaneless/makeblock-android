package com.iflytek.cloud.thirdparty;

import android.content.Context;
import android.text.TextUtils;
import com.iflytek.cloud.SpeechError;
import com.iflytek.msc.MSC;
import com.iflytek.msc.MSCSessionInfo;
import java.io.UnsupportedEncodingException;
import java.util.Date;

public class m
  extends C
{
  private MSCSessionInfo c = new MSCSessionInfo();
  private MSCSessionInfo d = new MSCSessionInfo();
  private byte[] e = null;
  
  private void a(byte[] paramArrayOfByte, int paramInt1, int paramInt2)
    throws SpeechError
  {
    try
    {
      ad.a("QISEAudioWrite enter");
      paramInt2 = MSC.QISEAudioWrite(this.a, paramArrayOfByte, paramInt1, paramInt2, this.d);
      ad.a("QISEAudioWrite leavel:" + paramInt2);
      this.c.sesstatus = this.d.sesstatus;
      ad.a("QISEAudioWrite length:" + paramInt1);
      if (paramInt2 != 0) {
        throw new SpeechError(this.d.errorcode);
      }
    }
    finally {}
  }
  
  public int a(Context paramContext, String paramString, B paramB)
    throws SpeechError, UnsupportedEncodingException
  {
    this.a = null;
    paramContext = ai.e(paramContext, paramB);
    ae.a("MSCSessionBegin", null);
    if (TextUtils.isEmpty(paramString)) {
      this.a = MSC.QISESessionBegin(paramContext.getBytes(paramB.p()), null, this.c);
    }
    int i;
    for (;;)
    {
      ae.a("SessionBeginEnd", null);
      i = this.c.errorcode;
      if ((i == 0) || (i == 10129) || (i == 10113) || (i == 10132)) {
        break;
      }
      throw new SpeechError(i);
      this.a = MSC.QISESessionBegin(paramContext.getBytes(paramB.p()), paramString.getBytes(paramB.p()), this.c);
      ad.a("sessionBegin userModelId:" + paramString);
    }
    return i;
  }
  
  public void a()
    throws SpeechError
  {
    try
    {
      ae.a("LastDataFlag", null);
      a(new byte[0], 0, 4);
      return;
    }
    finally
    {
      localObject = finally;
      throw ((Throwable)localObject);
    }
  }
  
  public void a(String paramString)
  {
    if (this.a == null) {
      return;
    }
    String str = paramString;
    if (TextUtils.isEmpty(paramString)) {
      str = "unknown";
    }
    ad.a("ISESessionEnd enter ");
    long l = System.currentTimeMillis();
    int i = MSC.QISESessionEnd(this.a, str.getBytes());
    ad.a("ISESessionEnd leavel:" + i + " time:" + (System.currentTimeMillis() - l));
    this.a = null;
    this.b = null;
  }
  
  public void a(byte[] paramArrayOfByte, int paramInt)
    throws SpeechError
  {
    try
    {
      a(paramArrayOfByte, paramInt, 2);
      return;
    }
    finally
    {
      paramArrayOfByte = finally;
      throw paramArrayOfByte;
    }
  }
  
  public void a(byte[] paramArrayOfByte1, byte[] paramArrayOfByte2)
    throws SpeechError
  {
    try
    {
      ad.a("QISETextPut enter");
      int i = MSC.QISETextPut(this.a, paramArrayOfByte1, paramArrayOfByte2);
      ad.a("QISETextPut leavel:" + i);
      if (i != 0) {
        throw new SpeechError(i);
      }
    }
    finally {}
  }
  
  public int b()
  {
    try
    {
      int i = this.d.epstatues;
      return i;
    }
    finally
    {
      localObject = finally;
      throw ((Throwable)localObject);
    }
  }
  
  public String b(String paramString)
  {
    Object localObject2 = null;
    for (;;)
    {
      try
      {
        localObject1 = this.a;
        if (localObject1 != null) {
          continue;
        }
        localObject1 = localObject2;
      }
      finally
      {
        try
        {
          if (MSC.QISEGetParam(this.a, paramString.getBytes(), this.c) != 0) {
            continue;
          }
          localObject1 = new String(this.c.buffer);
        }
        catch (Exception paramString)
        {
          Object localObject1 = localObject2;
        }
        paramString = finally;
      }
      return (String)localObject1;
      localObject1 = localObject2;
    }
  }
  
  public int c()
  {
    int j = 0;
    for (;;)
    {
      try
      {
        char[] arrayOfChar = this.a;
        if (arrayOfChar == null)
        {
          i = j;
          return i;
        }
      }
      finally {}
      try
      {
        i = MSC.QISEGetParam(this.a, "volume".getBytes(), this.d);
        if (i == 0) {}
        try
        {
          int k = Integer.parseInt(new String(new String(this.d.buffer)));
          i = k;
        }
        catch (Exception localException1) {}
        ad.a("VAD CHECK FALSE");
        i = j;
      }
      catch (Exception localException2)
      {
        i = 0;
        continue;
      }
      ad.a("getAudioVolume Exception vadret = " + i);
      i = j;
    }
  }
  
  public byte[] d()
  {
    return this.e;
  }
  
  public C.a e()
    throws SpeechError
  {
    Date localDate1 = new Date();
    this.e = MSC.QISEGetResult(this.a, this.c);
    Date localDate2 = new Date();
    StringBuilder localStringBuilder = new StringBuilder().append("QISRGetResult leavel:");
    if (this.e != null) {}
    int i;
    for (boolean bool = true;; bool = false)
    {
      ad.a(bool + " time:" + (localDate2.getTime() - localDate1.getTime()));
      i = this.c.errorcode;
      if (i != 0) {
        break;
      }
      i = this.c.rsltstatus;
      switch (i)
      {
      case 1: 
      case 3: 
      case 4: 
      default: 
        return C.a.c;
      }
    }
    ad.a("ResultStatus: hasResult" + i);
    return C.a.a;
    ad.a("ResultStatus: noResult" + i);
    return C.a.c;
    ad.a("ResultStatus: resultOver" + i);
    return C.a.f;
    ad.a("Result: error errorcode is " + i);
    throw new SpeechError(i);
  }
  
  protected String f()
  {
    if (this.b == null) {
      this.b = b("sid");
    }
    return this.b;
  }
}


/* Location:              C:\Users\Marcus Cheung\Desktop\AndriodHacking\Makeblock_v3.0.8_apkpure.com-dex2jar.jar!\com\iflytek\cloud\thirdparty\m.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */