  ...
  for (var i in topBlocks) {
    ...
    if (item && item.xmlData) {
      ...
      if (blockName.hasStr('when_')) {
        ...
        // 统计组件的各状态下是否具有代码
        var hascode = MBlockly.WhenEvent.WhenTable.widgetStateHasCode;
        hascode[id] = hascode[id] || {};
        if (topBlocks[i].getChildren().length > 0) {
          hascode[id][state] = true;
        } else {
          hascode[id][state] = false;
        }
...
MBlockly.WhenEvent.activateWhenBlocks = function(widgetId, cpValue) {
  ...
  cpValue = cpValue ? parseInt(cpValue).toString() : 0;
  ...
  if (targetCodes) {
    ...
    that.stopOtherState(widgetId, cpValue);
    runtime = new MBlockly.Runtime(widgetId);
    runtime.doInterpreter(xdecode(targetCodes[0]));
...
/**
 *
 * 停止当前组件的其他状态的运行，例如按钮按下和抬起，执行抬起动作时，如果抬起块下有额外代码，会终止按下的程序，否则不操作
 * @param  {number} widgetId     widget id
 * @param  {number} currentState current state
 */
MBlockly.WhenEvent.stopOtherState = function(widgetId, currentState) {
  if (!MBlockly.Settings.OPEN_STATE_MUTEX) {
    return;
  }

  function doStop() {
    for (var i in runtimeList) {
      if (i.split("-")[0] == widgetId && i.split("-")[1] != currentState) {
        runtimeList[i].stop();
      }
    }
  }

  // 检查当前state下有没有代码，如果有，则终止该id下其他state的runtime，如果没有，则不执行
  if (this.WhenTable.widgetStateHasCode[widgetId] && this.WhenTable.widgetStateHasCode[widgetId][currentState]) {
    doStop();
  }
};
