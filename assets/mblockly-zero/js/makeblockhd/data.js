...
MBlockly.Data = {
  ...
  noNeedCodingWidgets: ['MBWJoystick', 'MBWSpeaker', 'MBWColorPicker', 'MBWMusicKey', 'MBWFlightAttitudeJoystick',
        'MBWCarJoystick', 'MBWFlightJoystick', 'MBWPathController', 'MBWVerticalJoystick'],
  // 需要编程但不显示在顶部
    ...
    add: function(data) {
      ...
      if (!groups.ids[data.id]) {
        ...
        if(!MBlockly.Settings.DEBUG) {
          MBlockly.HostInterface.requestWidgetName(data.id, function(nameStr) {
              var name = nameStr + '-' + data.id;
              temp.push(name);
              temp.push(data.id);
