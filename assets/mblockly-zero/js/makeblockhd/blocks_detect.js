MBlockly.BlockKeeper.makeBlock('detect_get_slider_value', ['SLIDER'], function(){
    ...
    MBlockly.HostInterface.requestWidgetValue(sliderId, function(value) {
      valueWrapper.setValue(value)
      runtime.resume();
    })
    this.result = valueWrapper;
    return valueWrapper;
...
//红外传感器
//TODO: 红外传感器使用的软串口，所以该传感器不能同时连接两个
MBlockly.BlockKeeper.makeBlock('detect_infrared', ['PORT'], function(){
    var port = new Blockly.FieldDropdown(MBlockly.Data.portList.get());

    this.setColour(MBlockly.BlockKeeper.HUE.detect);
    this.appendDummyInput()
        // .appendField(new Blockly.FieldImage(MBlockly.Settings.resources().ICONS.DETECT_INFRARED,
         // MBlockly.Settings.BLOCK_ICON_WIDTH, MBlockly.Settings.BLOCK_ICON_HEIGHT, '*'))
        .appendField(Blockly.Msg.DETECT_INFRARED)
        .appendField(port, 'PORT');
    ...
    var runtime = this;
    runtime.pause();
    var val = MBlockly.Control.getSensorValue('INFRARED_SENSOR', port, function(result){
        runtime.resume();
    });
    return val;
}, Blockly.JavaScript.ORDER_FUNCTION_CALL);
...
// auriga temperature
MBlockly.BlockKeeper.makeBlock('detect_temperature', [], function(){
    this.setColour(MBlockly.BlockKeeper.HUE.detect);
    this.appendDummyInput()
    ...
    var port = MBlockly.Control.SETTING.PORT.auriga.TEMPERATURE;
    var runtime = this;
    runtime.pause(); // 暂停程序执行
...
MBlockly.BlockKeeper.makeBlock('detect_temperature_humidity', ['PORT', 'TYPE'], function(){
    ...
    this.setColour(MBlockly.BlockKeeper.HUE.detect);
    this.appendDummyInput()
        .appendField(new Blockly.FieldImage(MBlockly.Settings.resources().ICONS.DETECT_HUMIDITY,
         MBlockly.Settings.BLOCK_ICON_WIDTH, MBlockly.Settings.BLOCK_ICON_HEIGHT, '*'))
...
MBlockly.BlockKeeper.makeBlock('detect_common_temperature', ['PORT', 'SLOT'], function(){
    ...
    this.appendDummyInput()
        .appendField(new Blockly.FieldImage(MBlockly.Settings.resources().ICONS.DETECT_TEMPERATURE,
         MBlockly.Settings.BLOCK_ICON_WIDTH, MBlockly.Settings.BLOCK_ICON_HEIGHT, '*'))
        .appendField(Blockly.Msg.DETECT_COMMON_TEMPERATURE)
        .appendField(port, 'PORT')
...
//气体传感器，用于探测周围有害气体浓度
MBlockly.BlockKeeper.makeBlock('detect_gas_strength', ['PORT'], function(){
    var port = new Blockly.FieldDropdown(MBlockly.Data.portList.get("GAS"));

    this.setColour(MBlockly.BlockKeeper.HUE.detect);
    this.appendDummyInput()
        .appendField(new Blockly.FieldImage(MBlockly.Settings.resources().ICONS.DETECT_GAS,
         MBlockly.Settings.BLOCK_ICON_WIDTH, MBlockly.Settings.BLOCK_ICON_HEIGHT, '*'))
        .appendField(Blockly.Msg.DETECT_GAS_STRENGTH)
        .appendField(port, 'PORT');
    ...
    var runtime = this;
    runtime.pause();
    var val = MBlockly.Control.getSensorValue('GAS_STRENGTH_SENSOR', port, function(result){
        runtime.resume();
    });
    return val;
}, Blockly.JavaScript.ORDER_FUNCTION_CALL);
...
// 通用陀螺仪
MBlockly.BlockKeeper.makeBlock('detect_common_gyro', ['AXIS', 'PORT'], function(){
...
//电子罗盘
MBlockly.BlockKeeper.makeBlock('detect_compass', ['PORT'], function(){
    var port = new Blockly.FieldDropdown(MBlockly.Data.portList.get());//6，7，8，9，10

    this.setColour(MBlockly.BlockKeeper.HUE.detect);
    this.appendDummyInput()
        .appendField(new Blockly.FieldImage(MBlockly.Settings.resources().ICONS.DETECT_COMPASS,
         MBlockly.Settings.BLOCK_ICON_WIDTH, MBlockly.Settings.BLOCK_ICON_HEIGHT, '*'))
        .appendField(Blockly.Msg.DETECT_READ_COMPASS)
        .appendField(port, 'PORT');
    this.setInputsInline(true);
    this.setOutput(true, 'Number');
    this.setNextStatement(false);
    ...
    var runtime = this;
    runtime.pause(); // 暂停程序执行
    var val = MBlockly.Control.getSensorValue('COMPASS', port, function(){
        // 恢复程序执行
        runtime.resume();
    });
    return val;
