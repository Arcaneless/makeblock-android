...
(function(that) {
    ...
    // fixed: JSON.stringify 的Converting circular structure to JSON
    that.JSON.copyStringify = JSON.stringify;
    JSON.stringify = function(obj, replacer, spaces, cycleReplacer) {
      return that.JSON.copyStringify(obj, that.serializer(replacer, cycleReplacer), spaces);
    }

    that.serializer = function (replacer, cycleReplacer) {
      var stack = [], keys = []
