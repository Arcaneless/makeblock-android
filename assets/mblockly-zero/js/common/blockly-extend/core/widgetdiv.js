...
Blockly.WidgetDiv.position = function(anchorX, anchorY, windowSize,
  ...
  // Modified by Hyman
  if (anchorY < scrollOffset.y || anchorY > 0 && anchorY < $('#widgets').height()) {
 ...
 */
Blockly.WidgetDiv.addClassAndMask = function(className, always){
  var oldClass = Blockly.WidgetDiv.DIV.className;
  Blockly.WidgetDiv.showMask(always);
 ...
 */
Blockly.WidgetDiv.showMask = function(always){
  ...
  if(always){
    Blockly.WidgetDiv.MASK.bindData = [];
  }else{
 ...
 */
Blockly.WidgetDiv.show = function(newOwner, rtl, dispose, isCustomized, always) {
  ...
  if(isCustomized) {
    Blockly.WidgetDiv.addClassAndMask('widget-center', always);
  } else {
