...
Blockly.WorkspaceSvg.prototype.isDeleteArea = function(e) {
  ...
  if (this.deleteAreaToolbox_) {
    if (this.deleteAreaToolbox_.contains(xy)) {
      ...
      if(Blockly.onMouseOverToolboxDeletionZoneHandler){
        Blockly.onMouseOverToolboxDeletionZoneHandler(e);
      }
      return true;
    }
  }
  ...
  if(Blockly.onMouseOutToolboxDeletionZoneHandler){
    Blockly.onMouseOutToolboxDeletionZoneHandler(e);
  }
...
Blockly.WorkspaceSvg.prototype.setScale = function(newScale) {
  //modified by jeremy: when the scale arrives to the maxScale or the minScale,
  //set the button disabled
  if (this.options.zoomOptions.maxScale &&
      newScale >= this.options.zoomOptions.maxScale) {
    newScale = this.options.zoomOptions.maxScale;
    //set disabled
    var zoomoutSvg = document.querySelector('.blocklyZoom .zoomout');
    zoomoutSvg.setAttribute('x', zoomoutSvg.getAttribute('data-disabled'));
    Blockly.addClass_(zoomoutSvg, 'disabled');
  } else if (this.options.zoomOptions.minScale &&
      newScale <= this.options.zoomOptions.minScale) {
    newScale = this.options.zoomOptions.minScale;
    //set disabled
    var zoominSvg = document.querySelector('.blocklyZoom .zoomin');
    zoominSvg.setAttribute('x', zoominSvg.getAttribute('data-disabled'));
    Blockly.addClass_(zoominSvg, 'disabled');
  }
  this.scale = newScale;
  this.updateGridPattern_();
  if (this.scrollbar) {
