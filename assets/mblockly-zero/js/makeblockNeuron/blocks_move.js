...
/* 设置智能舵机群组块 */
MBlockly.BlockKeeper.makeBlock('smartServo_list_set_absolute_angle', ['SERVO_LIST'], function() {
  var smartServoList = new MBlockly.SmartServoList(this.id);

  this.setColour(MBlockly.BlockKeeper.HUE.move);
  this.appendDummyInput()
    .appendField('set smart servos in a group')
    .appendField(smartServoList, "SERVO_LIST");

  this.setInputsInline(true);
  this.setNextStatement(true);
  this.setPreviousStatement(true);
}, function(blockId) {
  var data = MBlockly.Data.getOne(MBlockly.App.currentWidget.id)[blockId.data];
  var delay = 10;

  for (var i in data) {
    var itemData = data[i];
    var abAngle = Math.abs(parseInt(itemData.angle));
    if (abAngle <= 360) {
      MBlockly.Servo.setServoAbsoluteShortAngle(itemData.index, itemData.angle, itemData.speed);
    } else {
      MBlockly.Servo.setServoAbsoluteLongAngle(itemData.index, itemData.angle, itemData.speed);
    }
  }
});
