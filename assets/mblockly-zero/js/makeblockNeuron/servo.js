...
extend(MBlockly.Servo, {
  ...
  bluetoothConnect: function() {
    MBlockly.Servo.startHeartbeatPkg();

    setTimeout(function() {
      MBlockly.Servo.stopHeartbeatPkg();
    }, 5000);
  },
   ...
   *   对于获取value的command，params表示上报模式
   *      底层固件默认是一条都不上报
   *      0x00表示收到查询命令时返回一次: 查询命令同时也是该设置命令 要想不回值，不进行模式设置
   *      0x01表示数据状态有变化时上报
   *      0x02表示周期上报: 10ms会发送一次
  ...
  // 打开心跳包
  startHeartbeatPkg: function() {
    MBlockly.Neurons.engine.start();
  },

  // 关闭心跳包
  stopHeartbeatPkg: function() {
    MBlockly.Neurons.engine.stop();
  },
